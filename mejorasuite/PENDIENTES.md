...
- [ ] **KPI de campañas en Reports.tsx** — nueva sección con interacciones `medium = 'whatsapp'`, reusando `businessLogic.ts` + recharts. Limitado a lo que hay: respuestas entrantes detectadas automáticamente, sin "tasa de respuesta" real (no hay conteo de enviados, ver límite conocido de la Fase 4 arriba).
- [ ] **Modo Demostración aislado** — vista nueva (`/demo` o similar) que renderiza directo desde `src/demo/demoData.ts`, sin tocar `DEMO_MODE` ni ningún hook real (esa constante es intencionalmente no-reactiva, documentado en `CLAUDE.md` de este repo — no se toca).
- [ ] **Consultas en lenguaje natural sobre el CRM** — panel en Reports.tsx, clave de API de IA cifrada en `localStorage` (mismo patrón que `MejoraContactos/src/lib/api-keys.ts`), resume KPIs ya calculados + pregunta libre, llama al proveedor de IA directo desde el browser. Bloqueante: necesita que Pablo pegue su propia clave de API una vez — no es algo que Claude pueda generar.
